...
random_data0=lambda:"".join(("{\"phoneNumber\":\"",phone,"\",\"roleType\":",roleType,"}"))
random_data1=lambda code:"".join(("{\"phoneNumber\":\"",phone,"\",\"roleType\":",roleType,",\"code\":\"",code,"\"}"))
DEFAULT_POOLSIZE=255
roleType="1"
...
captcha1_headers={"accept":match_headers["accept"],
"accept-encoding":match_headers["accept-encoding"],
"accept-language":match_headers["accept-language"],
"content-length":"56",
...
def send_loop(lock,write):
    try:
        ...
        if json["code"]==0:
            ...
            label.tag_captcha0
            #第一次请求,判断是否需要填图形验证码
            response=connect.post("https://hfs-be.yunxiao.com/v2/native-users/verification-msg-with-captcha",headers=captcha0_headers,data=random_data0())
            ###debug###
            #print(response.text)
            #print(str(response.json()["code"])+":"+response.json()["msg"])
            #sleep(1)
            ...
            if code==0:
                ...
                label.tag_captcha1
                #要填图形验证码的请求
                response=connect.post("https://hfs-be.yunxiao.com/v2/native-users/verification-msg-with-captcha",headers=captcha1_headers,data=random_data1(get_svg_captcha(json["data"]["pic"])))
                ###debug###
                #print(response.text)
                #print(str(response.json()["code"])+":"+response.json()["msg"])
                #sleep(1)
...
if __name__=="__main__":#命令行调用,"python main.py"或"python main.py poolsize=[int] roleType=[byte] chrome_version=[str] threads=[int] logfile=[str] print=[bool]",每个参数都是可选的,如有重复后面的会覆盖前面的,poolsize连接池大小默认255,roleType默认"1",chrome_version默认"96.0.4664.93",threads线程数量默认1,logfile日志文件路径默认打印到控制台,print默认将log打印到控制台即使logfile不是默认
    ...
    if len(argv)==1:
        lock=allocate_lock()
        lock.acquire()
        start_new_thread(send_loop,(lock,stdout.write))
        ...
        for a in range(threads):
            ...
            lock.acquire()
            locks.append(lock)
            start_new_thread(send_loop,(lock,write))
            write("start thread\n")
        while True:
            a=0
            while a<threads:
                if not locks[a].locked():
                    write("end thread\n")
                    locks[a].acquire()
                    start_new_thread(send_loop,(locks[a],write))
                    write("start thread\n")
                a=a+1
                sleep(1)